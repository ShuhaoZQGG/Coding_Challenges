...
func HandleDefault(args []string, store *models.Store) (response string, err error) {
	response, err = Serialize[models.SimpleString](*models.NewSimpleString("OK"))
	return response, err
}

func HandlePing(args []string, store *models.Store) (response string, err error) {
	response, err = Serialize[models.SimpleString](*models.NewSimpleString("PONG"))
	return response, err
}

func HandleSet(args []string, store *models.Store) (response string, err error) {
	store.Set(args[0], args[1])
	response, err = Serialize[models.SimpleString](*models.NewSimpleString("OK"))
	return response, err
}

func HandleGet(args []string, store *models.Store) (response string, err error) {
...
func HandleExists(args []string, store *models.Store) (response string, err error) {
...
func HandleDelete(args []string, store *models.Store) (response string, err error) {
	response = SerializeIntegers(int64(0))
	if len(args) == 1 {
		ok := store.Del(args[0])
		if ok {
